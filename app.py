...
_admin_id_list = [int(x.strip()) for x in (os.getenv("ADMIN_IDS") or "").split(",") if x.strip().isdigit()]
ADMIN_IDS = set(_admin_id_list)
_owner_raw = (os.getenv("OWNER_ID") or "").strip()
# OWNER_ID wins when configured. Otherwise the first ID written in ADMIN_IDS is
# the primary owner. This keeps private owner controls available after deploy
# without exposing them to every administrator.
OWNER_ID = int(_owner_raw) if _owner_raw.isdigit() else (_admin_id_list[0] if _admin_id_list else None)
...
STOCKS.update({
    ...
    "google": {"symbol":"GOOGL","name":"Google","description":"Google развивает поиск и рекламу, YouTube, Android, Google Cloud и продукты на базе искусственного интеллекта.","min_price":250.0,"max_price":900.0,"initial_price":480.0,"volatility":.018,"drift":.0015,"dividend_rate":.004},
...
async def corporation_security_guard(request: Request, call_next):
    ...
    elif path.startswith("/static/news/"):
        # News artwork is versioned in its URL; cache it aggressively so the feed
        # opens instantly after the first request instead of re-downloading photos.
...
# === CORPORATION LOAD TEST V4 START ===

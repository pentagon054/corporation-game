...
$backup = Join-Path (Split-Path $ProjectPath -Parent) ((Split-Path $ProjectPath -Leaf) + '_before_v32_1_' + (Get-Date -Format 'yyyyMMdd_HHmmss_fff'))
...
$readmeSource = Join-Path $PSScriptRoot 'README_V32_1_RU.md'
if (Test-Path -LiteralPath $readmeSource) {
    Copy-Item -LiteralPath $readmeSource -Destination (Join-Path $ProjectPath 'README_V32_1_RU.md') -Force
}

Write-Host ''
Write-Host 'Corporation v32.1 hotfix installed successfully.' -ForegroundColor Green
...
Write-Host 'The startup race and subscription-screen flash were fixed.'

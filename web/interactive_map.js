...
class CorporationAtlas {
  ...
  clamp(){this.scale=Math.min(32,Math.max(1,this.scale));const w=this.width*this.scale,h=this.width/2*this.scale;this.tx=w<=this.width?(this.width-w)/2:Math.max(this.width-w,Math.min(0,this.tx));this.ty=h<=this.height?(this.height-h)/2:Math.max(this.height-h,Math.min(0,this.ty));}
  zoom(scale,x=this.width/2,y=this.height/2){const next=Math.max(1,Math.min(32,scale)),ratio=next/this.scale;this.tx=x-(x-this.tx)*ratio;this.ty=y-(y-this.ty)*ratio;this.scale=next;this.closePicker();this.draw();}

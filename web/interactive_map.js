/* Corporation 26 atlas: vector land is resized instead of bitmap-scaled; pins stay on a transformed interaction plane. */
class CorporationAtlas {
  constructor(root,cities,{onSelect,saved,onChange}={}){
    ...
    root.innerHTML='<div class="interactive-map-viewport" tabindex="0" role="group" aria-label="Интерактивная карта. Города закреплены точками. Стрелки перемещают карту, плюс и минус меняют масштаб."><img class="interactive-map-land" src="/static/world.svg" alt="" draggable="false"><div class="interactive-map-scene"><div class="interactive-map-markers"></div></div></div><div class="interactive-map-tools"><button type="button" data-map-action="out" aria-label="Уменьшить карту">−</button><button type="button" data-map-action="reset" aria-label="Показать весь мир">◎</button><button type="button" data-map-action="in" aria-label="Увеличить карту">+</button></div><span class="interactive-map-scale" aria-live="off"></span><div class="interactive-map-picker" hidden></div>';
    this.viewport=root.querySelector('.interactive-map-viewport');this.land=root.querySelector('.interactive-map-land');this.scene=root.querySelector('.interactive-map-scene');this.markers=root.querySelector('.interactive-map-markers');this.picker=root.querySelector('.interactive-map-picker');
    this.buildMarkers();
  ...
  clamp(){this.scale=Math.min(12,Math.max(1,this.scale));const w=this.width*this.scale,h=this.width/2*this.scale;this.tx=w<=this.width?(this.width-w)/2:Math.max(this.width-w,Math.min(0,this.tx));this.ty=h<=this.height?(this.height-h)/2:Math.max(this.height-h,Math.min(0,this.ty));}
  zoom(scale,x=this.width/2,y=this.height/2){const next=Math.max(1,Math.min(12,scale)),ratio=next/this.scale;this.tx=x-(x-this.tx)*ratio;this.ty=y-(y-this.ty)*ratio;this.scale=next;this.closePicker();this.draw();}
  ...
  draw(){this.clamp();const mapW=this.width*this.scale,mapH=(this.width/2)*this.scale;this.land.style.left=this.tx+'px';this.land.style.top=this.ty+'px';this.land.style.width=mapW+'px';this.land.style.height=mapH+'px';this.scene.style.width=this.width+'px';this.scene.style.height=this.width/2+'px';this.scene.style.setProperty('--inverse-scale',String(1/this.scale));this.scene.style.transform=`translate3d(${this.tx}px,${this.ty}px,0) scale(${this.scale})`;this.scene.classList.toggle('detailed',this.scale>=2.25);this.root.querySelector('.interactive-map-scale').textContent=this.scale.toFixed(1)+'×';this.root.querySelector('[data-map-action="out"]').disabled=this.scale<=1;this.root.querySelector('[data-map-action="in"]').disabled=this.scale>=12;this.onChange?.(this.snapshot());}

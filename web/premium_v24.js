...
renderRealEstate=async function(){
  ...
  try{
    ...
    c.innerHTML=`<section class="realestate-page"><div class="eyebrow">ЧАСТНЫЙ ПОРТФЕЛЬ</div><h2>Недвижимость мира</h2><p class="section-note">Найди место для следующей инвестиции.</p><div class="interactive-atlas"></div><p class="map-gesture-note">Перетаскивай карту • Приближай двумя пальцами или кнопками +/−<br>На ПК: колесо — прокрутка страницы, Ctrl + колесо — масштаб карты.<br>Нажми на метку, чтобы открыть город.</p><p class="section-note map-catalog-heading">Все города</p><div class="city-selector">${cities.map(p=>`<button data-city="${p.city_id}" onclick="openCity('${p.city_id}')"><span>${escapeHTML(p.country)}</span><strong>${escapeHTML(p.city)}</strong><b>↗</b></button>`).join('')}</div><div class="realestate-summary"><span>В портфеле</span><b>${state.real_estate_count||0} объектов</b><span>Рост стоимости</span><b>+1% каждые 12 ч</b></div></section>`;

...
$backup = Join-Path (Split-Path $ProjectPath -Parent) ((Split-Path $ProjectPath -Leaf) + '_before_v32_3_' + (Get-Date -Format 'yyyyMMdd_HHmmss_fff'))
...
$readmeSource = Join-Path $PSScriptRoot 'README_V32_3_RU.md'
if (Test-Path -LiteralPath $readmeSource) {
    Copy-Item -LiteralPath $readmeSource -Destination (Join-Path $ProjectPath 'README_V32_3_RU.md') -Force
}

Write-Host ''
Write-Host 'Corporation v32.3 hotfix installed successfully.' -ForegroundColor Green

...
(async()=>{
 ...
 await page.locator('[data-map-key*="london"]').click();await page.locator('[data-cluster-city="london"]').click();
 assert.equal(await page.evaluate(()=>selected.at(-1)),'london');
 await page.locator('.map-picker-close').click();
 await page.locator('[data-map-key*="paris"]').click();await page.locator('[data-cluster-city="paris"]').click();
